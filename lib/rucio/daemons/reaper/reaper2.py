...
def get_rses_to_process(rses, include_rses, exclude_rses, vos):
    ...
    if rses:
        cache_key += '-%s' % str(rses)
    if include_rses:
        cache_key += '-%s' % str(include_rses)
    if exclude_rses:
        cache_key += '-%s' % str(exclude_rses)
    if vos:
        cache_key += '@%s' % str(vos)
    ...
    vo = config_get('client', 'vo', raise_exception=False, default='def')
    if not multi_vo:
        if vos:
        ...
        if vos and vos != [vo]:
            logging.warning('Ignoring argument vos, cannot run the reaper on a different VO.')
        vos = [vo]
        logging.info('Reaper: This instance will work on VO: %s' % vo)
    else:
        if vos:
